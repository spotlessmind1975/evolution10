...
3r=rN(1):rm=rN(1):goS8:x$=k$:?"{white} + ";:goS8:y$=k$:?:?:on-(x$<>y$)gO4:?"{red}you have to choose different phenotypes!{white}":gO3
4?:i=vA(x$):goS9:?"{white}";mI(n$,i,1);"]";f$;:?"{white} + ";:i=vA(y$):goS9:?"{white}";mI(n$,i,1);"]";f$:?:fOi=1tog:d(1,i)=c(vA(x$),i)
5d(2,i)=c(vA(y$),i):nEi:fOi=3toe-1:fOj=1tog:d(i,j)=d(1-(rN(1)>.5),j):nEj:nEi:on-(r>.8)gO6:d(3-(rm>.5),rk)=1+int(rN(1)*3)
